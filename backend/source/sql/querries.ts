...
const login = async (params: string[]) => {
    const x = await execute("SELECT ? = (SELECT PASSWORD FROM USER WHERE EMAIL = ?) AS login, UID FROM USER WHERE EMAIL = ? AND PASSWORD = ?;", params)
    console.log(x);
    return x
};
...
const searchByActor = async (params: string[]) => {
    const x = await execute("SELECT A.AID, A.NAME FROM ACTOR A WHERE A.NAME LIKE CONCAT('%', ?, '%');", params);
    console.log(x);
    return x;
};
...
const getActorByAid = async (params: string[]) => {
    const x = await execute('SELECT AID, NAME, CREATED, UPDATED FROM ACTOR WHERE AID = ?', params);
    console.log(x);
    return x;
};
...
const getWatchList = async (params: string[]) => {
    const x = execute("SELECT WE.MPID AS MPID, M.TITLE AS TITLE FROM WATCHLISTENTRY WE JOIN WATCHLIST W on WE.WLID = W.WID JOIN MOTIONPICTURE M on WE.MPID = M.MPID WHERE W.UID = ?;", params);
    console.log(x);
    return x;
};
...
const getAllActors = async (params: string[]) => {
    return await execute("SELECT AID, NAME FROM ACTOR", params);
};

const getAllMovies = async (params: string[]) => {
    return await execute("SELECT MPID, TITLE FROM MOTIONPICTURE", params);
};

const getUserInfo = async(params: string[]) => {
  return await execute("SELECT FNAME, LNAME, EMAIL FROM USER WHERE UID = ?", params)
};
